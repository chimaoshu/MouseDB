...
{
    string current_used_hot_data_file_path = table_meta_handler_->get_current_used_hot_data_file_name();
    string new_hot_data_file_path = table_meta_handler_->get_new_hot_data_file_name();
    ...
    if (current_used_hot_data_file_path.empty() || hot_dump)
    {
        ...
        current_used_hot_data_file_ = new TableRowHandler(new_hot_data_file_path, table_meta_handler_, 1, 1);
...
status_code HotDataManager::insert_rows(const json &rows_info)
{
    ...
    if (!err)
    {
        ...
        // 遍历每一行数据
        for (auto it_of_rows = rows_info.begin(); it_of_rows != rows_info.end(); it_of_rows++)
        {
...
HotDataManager *HotDataManager::dump_to_cold_data(bool is_recovery)
{
    ...
    if (!is_recovery)
    {
        this->new_hot_data_manager_ = new HotDataManager(
            table_dir_,
            table_meta_handler_);
    }
    ...
    string current_used_cold_data_file_path =
        table_dir_ + '/' + table_meta_handler_->get_current_used_cold_data_file_name();
    ...
    string new_cold_data_file_name = table_dir_ + Tools::get_str_timestamp() + ".mdb";
    TableRowHandler new_cold_data_file(
        new_cold_data_file_name,
        table_meta_handler_,
        true,
        0);
    ...
    if (it_of_rbtree == end_of_rbtree)
    {
        ...
        if (the_rest_of_the_file.second != 0)
            new_cold_data_file.write_rows(the_rest_of_the_file.first, the_rest_of_the_file.second);
