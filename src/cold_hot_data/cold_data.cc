...
status_code ColdDataManager::build_index() {
  ...
  if (row_number_of_cold_data_file_ % index_interval_rows_number_ == 0)
    index_.resize(row_number_of_cold_data_file_ / index_interval_rows_number_,
                  rbtree_value());
...
ColdDataManager::ColdDataManager(const string &table_dir,
  ...
  if (cold_data_file_name.empty())
    ...
    cold_data_file_path =
        table_dir + '/' +
        table_meta_handler->get_current_used_cold_data_file_name();

  else
    cold_data_file_path = table_dir + '/' + cold_data_file_name;

  cold_data_file_ =
      new TableRowHandler(cold_data_file_path, table_meta_handler, 0, 1);

  this->build_index();
}

ColdDataManager::~ColdDataManager() { delete cold_data_file_; }

// TODO 以后加个min<answer<max，需要连同前面的start_row、end_row一起改
// 目前是min <= answer <= max
// 返回符合条件的行在冷数据文件中的行数
list<row_order>
ColdDataManager::find_values_between_primary_keys(const rbtree_key &min,
  ...
  for (row_order current_row = start_scanning_row;
       current_row <= end_scanning_row; current_row++) {
