...
DatabaseHandler::DatabaseHandler(DatabaseMetaHandler &database_meta_handler,
                                 string &database_meta_dir)
...
TablesHandler *DatabaseHandler::open(const string &database_name) {
  ...
  if (it == database_meta_.end()) {
    ...
    TablesHandler *table_handler =
        new TablesHandler(database_name, database_meta_);
...
status_code DatabaseHandler::drop(const string &database_name) {
  ...
  if (i == database_meta_.end()) {
    ...
    // 遍历每一个表
    for (auto it = database_meta_[database_name].begin();
         it != database_meta_[database_name].end(); it++) {
