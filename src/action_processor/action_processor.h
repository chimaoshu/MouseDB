...
public:
  ...
  // 创建一个表
  void create_a_table(const std::string &table_name,
  ...
  void get_lines(uint32_t off_set, uint32_t line_number,
                 const std::string &table_name,
                 std::list<std::string> &wanted_column_names, bool debug_mode);
};
#endif