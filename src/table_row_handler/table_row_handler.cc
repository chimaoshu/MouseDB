...
template <class T>
T *TableRowHandler::read_and_deserialize(uint32_t &off_set,
                                         uint32_t &line_number,
                                         list<int> wanted_columns) {

  pair<void *, uint32_t> buffer_info = file_.read_lines_into_buffer(
      off_set * line_size_, line_size_, line_number, false, false);
...
status_code TableRowHandler::serialize_and_write(const T &rows_information) {
  ...
  for (auto it_of_rows = rows_information.begin();
       it_of_rows != rows_information.end(); it_of_rows++, row_order++) {
    ...
    for (auto it = row.begin(); it != row.end(); it++, column_order++) {
      ...
      // 计算从buffer写入该变量的初始地址
      int off_set_address =
          line_size_ * row_order + off_set_of_each_column_[column_order];
...
pair<void *, rbtree_key *> TableRowHandler::read_next_row_buffer_and_index() {
  ...
  pair<void *, uint32_t> buffer_info =
      file_.read_lines_into_buffer(0, line_size_, 1, true, false);
...
rbtree_key *TableRowHandler::read_row_index(int row_order) {
  ...
  if (row_order < 0) {
    ...
    buffer = file_.read_primary_key_into_buffer(
        0, primary_key_number_ * sizeof(primary_key_type), true);
  ...
  // 传入参数不小于0，表示读某一行
    ...
    buffer = file_.read_primary_key_into_buffer(
        (row_order - 1) * line_size_,
        primary_key_number_ * sizeof(primary_key_type), false);
...
void *TableRowHandler::read_row_buffer(row_order line_order) {
  ...
  pair<void *, uint32_t> buffer_info = file_.read_lines_into_buffer(
      (line_order - 1) * line_size_, line_size_, 1, false, false);
...
// 特化
template json *TableRowHandler::read_and_deserialize(uint32_t &off_set,
                                                     uint32_t &line_number,
                                                     list<int> wanted_columns);
template list<json> *
