...
TablesHandler::TablesHandler(const string &database_name,
                             const json &database_meta)
  ...
  for (auto it = database_meta[database_name].begin();
       it != database_meta[database_name].end(); it++) {
    table_name = it.key();
    table_dir = it.value();

    string table_meta_path = table_dir + '/' + table_name + ".json";
    map_of_table_name_to_table_meta_handler_[table_name] =
        new TableMetaHandler(table_meta_path);
  }
}
...
TablesHandler::~TablesHandler() {
  ...
  for (auto it = map_of_table_name_to_table_meta_handler_.begin();
       it != map_of_table_name_to_table_meta_handler_.end(); it++) {
    // 释放TeblaMetaHandler
    if (it->second)
      delete it->second;
  }
  ...
  for (auto it = map_of_table_name_to_hot_data_manager_.begin();
       it != map_of_table_name_to_hot_data_manager_.end(); it++) {
...
StatusCode TablesHandler::create_table(const string &table_directory,
  ...
  db_mete_handler.add_table_in_json(database_name_, table_name,
                                    table_directory);
  ...
  map_of_table_name_to_table_meta_handler_[table_name] =
      new TableMetaHandler(table_meta_path, table_meta);
...
HotDataManager *&
TablesHandler::get_hot_data_manager(const std::string &table_name) {
  ...
  // 不存在则创建
    ...
    // 热数据文件的路径
    string hot_data_file_path =
        this->get_table_dir(table_name) + '/' + table_name + ".hot";

    // 打开table_row_handler
    map_of_table_name_to_hot_data_manager_[table_name] = new HotDataManager(
        hot_data_file_path, this->get_table_meta_handler(table_name));
...
ColdDataManager *&
TablesHandler::get_cold_data_manager(const std::string &table_name) {
  ...
  // 不存在则创建
    ...
    map_of_table_name_to_cold_data_manager_[table_name] = new ColdDataManager(
        cold_data_file_path, this->get_table_meta_handler(table_name));

    return map_of_table_name_to_cold_data_manager_[table_name];
  }
}

void TablesHandler::set_hot_data_manager_in_map(
    const string &table_name, HotDataManager *new_hot_data_manager) {
  map_of_table_name_to_hot_data_manager_[table_name] = new_hot_data_manager;
}

void TablesHandler::set_cold_data_manager_in_map(
    const string &table_name, ColdDataManager *new_cold_data_manager) {
...
StatusCode TablesHandler::dump_table_in_sub_thread(const string &table_name) {
  ...
  HotDataManager *new_hot_data_manager =
      old_hot_data_manager->dump_to_cold_data();

  // 获取现在table对应的TableMetaHandler
  TableMetaHandler *current_table_meta_handler =
  ...
  ColdDataManager *new_cold_data_manager = new ColdDataManager(
      this->get_table_dir(table_name), current_table_meta_handler);
