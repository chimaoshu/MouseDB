...
private:
  string test_dir =
      string(get_current_dir_name()) + "/test/handler_unit_test/table_handler";
...
public:
  ...
  TablesHandlerUnitTest()
      : nothing(remove(test_dir.c_str())), ua(UserAction(test_dir)) {
  ...
  void test_drop_table() { ua.drop_a_table("student"); }
  ...
  ~TablesHandlerUnitTest() { ua.drop_a_database("foo"); }
};
